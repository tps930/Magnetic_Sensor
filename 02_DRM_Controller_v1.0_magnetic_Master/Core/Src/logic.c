...
#define T_ABS_MIN_FLOOR              30      // 임계값 하한
#define T_ABS_MAX_CEIL              400      // 임계값 상한
#define K_SIGMA_INIT               4.5f      // 초기 보정 시 사용

#define T_HYST_RATIO_PERMILLE       600      // 종료임계 = 진입임계 * 0.6
#define EVT_MIN_MS                   70      // 너무 짧은 이벤트 무시
#define EVT_MIN_AREA               1200      // 면적(합) 너무 작으면 무시
#define REFRACT_MS                 1200      // 종료 후 불응시간

#define AUTOTUNE_INTERVAL_MS       5000      // 5초마다 자동보정 1회
#define EMA_BETA_NOISE_PERMILLE      50      // idle 구간 |rel| EMA
#define IDLE_BAND_DIV                10      // 기존 코드의 idle band 유지
...
#define D_SLOPE_MIN              4     // 기울기 최소치(필드에서 조정)
#define PRE_LATCH_DELTA_T        8     // 래치 동안 진입 임계를 임시로 낮추는 폭
#define PRE_LATCH_MAX_MS       200     // 래치 유지 최대 시간
#define RMS_WIN                 16     // 짧은 RMS 창 길이(샘플 수)
#define RMS_MIN                160     // RMS(에너지) 하한 (필드 조정)

#define SPEED_SEC_MIN     (0.020f)            // 20ms 이하 무시 (이상치 방지)
#define SPEED_KMH_MAX     (255.0f)            // 8bit 전송 대비 상한
#define SPEED_LP_ALPHA_PM 200                 // 0~1000 (0.2 저역통과 필터 강도)
static float s_speed_lpf = 0.0f;             // 속도 LPF 상태
...
{
	printf("======================================\r\n");
	printf("  Magnetic Controller v1.0(Master)\r\n");
	printf("  LEDO Co,. Ltd. \r\n");
	printf("======================================\r\n");
...
void Send_BLE_MANUF_SET()
{
   ...
   sprintf(ls.ble.command_str, "AT+MANUF=MASTER\r");
...
static TickType_t s_evt_start_tick = 0;
static int32_t    s_evt_area       = 0;
static TickType_t s_last_autotune  = 0;
static uint16_t   s_q_true         = 0;   // 정상 이벤트 개수
static uint16_t   s_q_false        = 0;   // 가짜 이벤트 개수
static float      s_noise_ema_abs  = 0.0f; // idle구간 |rel| EMA

static int16_t  s_prev_rel = 0;
static uint8_t  s_pre_latch_on = 0;
static TickType_t s_pre_latch_until = 0;

static int16_t  s_ring[RMS_WIN];
static uint8_t  s_ring_i = 0;
static int32_t  s_ring_acc2 = 0; // ∑(rel^2)

static inline int16_t T_lo_from(int16_t T_hi)
{
    return (int16_t)((T_hi * T_HYST_RATIO_PERMILLE)/1000);
}

static inline uint16_t small_rms_push_and_get(int16_t x){
    // ring buffer에 x^2 누적
    int16_t old = s_ring[s_ring_i];
    s_ring_acc2 -= (int32_t)old*old;
    s_ring[s_ring_i] = x;
    s_ring_acc2 += (int32_t)x*x;
    s_ring_i++; if (s_ring_i >= RMS_WIN) s_ring_i = 0;

    // 고정소수점 근사 sqrt
    float rms = sqrtf((float)s_ring_acc2 / (float)RMS_WIN);
    return (uint16_t)lrintf(rms);
}
...
void Magnetic_Sensor_Init(void)
{
	   uint8_t who = 0, reg = 0, status = 0;
	    ...
	    //uint8_t cfg_a = 0x80;	// 10hz
	    //uint8_t cfg_a = 0x84; // 20hz
	    //uint8_t cfg_a = 0x88; // 50hz
	    uint8_t cfg_a = 0x8C; // 100hz
...
void MagProc_Calibrate_from_buf(const int16_t* buf, int n)
{
    ...
    ls.magnetic.Z0    = (int32_t)llroundf(mean);
    ls.magnetic.sigma = sigma;

    // 초기 임계값 = max(MIN, K_SIGMA_INIT * sigma) (가드레일 적용)
    int16_t T = (int16_t)lrintf(fmaxf((float)T_ABS_MIN_FLOOR, K_SIGMA_INIT * sigma));
    if (T < T_ABS_MIN_FLOOR) T = T_ABS_MIN_FLOOR;
    if (T > T_ABS_MAX_CEIL ) T = T_ABS_MAX_CEIL;
    ls.magnetic.T = T;
    ...
    s_evt_start_tick = 0;
    s_evt_area       = 0;
    s_last_autotune  = xTaskGetTickCount();
    s_q_true = s_q_false = 0;
    s_noise_ema_abs  = ls.magnetic.sigma; // 초기값은 sigma로 시작

    printf("[MAG] Calib |B|: M0=%ld, sigma=%.2f, T=%d\r\n",
           (long)ls.magnetic.Z0, ls.magnetic.sigma, ls.magnetic.T);
}

static void MagProc_Autotune_idle(void)
{
    const TickType_t now = xTaskGetTickCount();
    if ((now - s_last_autotune) < pdMS_TO_TICKS(AUTOTUNE_INTERVAL_MS))
        return;
    s_last_autotune = now;

    // idle동안 추적한 |rel|의 EMA 기반 권장 T
    int16_t suggested = (int16_t)lrintf(fmaxf((float)T_ABS_MIN_FLOOR, 4.5f * s_noise_ema_abs));
    if (suggested < T_ABS_MIN_FLOOR) suggested = T_ABS_MIN_FLOOR;
    if (suggested > T_ABS_MAX_CEIL ) suggested = T_ABS_MAX_CEIL;

    // 가짜 이벤트가 많으면 T를 빠르게 올림
    if (s_q_false >= 3 && ls.magnetic.T < T_ABS_MAX_CEIL) {
        ls.magnetic.T += 4;
        s_q_false = 0;
    }
    // 정상 이벤트가 연속으로 많으면 T를 조금 낮춤(민감도 +)
    else if (s_q_true >= 5 && ls.magnetic.T > T_ABS_MIN_FLOOR) {
        ls.magnetic.T -= 2;
        s_q_true = 0;
    }
    // 그 외에는 suggested에 천천히 수렴
    else {
        if (ls.magnetic.T < suggested - 2) ls.magnetic.T++;
        else if (ls.magnetic.T > suggested + 2) ls.magnetic.T--;
    }

    // 가드레일
    if (ls.magnetic.T < T_ABS_MIN_FLOOR) ls.magnetic.T = T_ABS_MIN_FLOOR;
    if (ls.magnetic.T > T_ABS_MAX_CEIL ) ls.magnetic.T = T_ABS_MAX_CEIL;

    // 디버그
    //printf("[MAG] autotune: T=%d, suggested=%d, true=%u, false=%u, ema=%.1f\n",ls.magnetic.T, suggested, s_q_true, s_q_false, s_noise_ema_abs);
}
void Magnetic_DRDY()
{
	uint8_t reg_val;
...
void MagProc_Process_sample(int16_t rec_mag, TickType_t peer_tick_or_0)
{
    ...
    // --- 기존 idle 근처 baseline 추적/EMA는 그대로 ---

    // [A] SlopeGate: 기울기 측정
    int16_t d = rel - s_prev_rel; if (d < 0) d = -d;

    // [B] EnergyGate: 짧은 RMS
    uint16_t rms = small_rms_push_and_get(rel);

    // [C] 소프트 래치 ON 조건: 진폭은 T_hi 미만이어도, 기울기/에너지 충분하면 래치
    if (ls.magnetic.st == WAIT_FALL && !s_pre_latch_on) {
        if (d >= D_SLOPE_MIN && rms >= RMS_MIN) {
            s_pre_latch_on = 1;
            s_pre_latch_until = t + pdMS_TO_TICKS(PRE_LATCH_MAX_MS);
            // printf("[MAG] pre-latch on: d=%d, rms=%u\n", d, rms);
        }
    }
    // 래치 시간 초과 시 자동 해제
    if (s_pre_latch_on && t >= s_pre_latch_until) {
        s_pre_latch_on = 0;
    }

    // 히스테리시스 임계
    const int16_t T_hi_base = ls.magnetic.T;
    const int16_t T_lo = T_lo_from(T_hi_base);

    // 래치가 켜져 있으면, 진입 임계만 약간 낮춰 봄(가드 포함)
    int16_t T_hi = T_hi_base;
    if (s_pre_latch_on) {
        int16_t tmp = T_hi_base - PRE_LATCH_DELTA_T;
        if (tmp < T_lo + 2) tmp = T_lo + 2;     // 너무 과하게 낮추지 않기
        if (tmp < T_ABS_MIN_FLOOR) tmp = T_ABS_MIN_FLOOR;
        T_hi = tmp;
    }

    switch (ls.magnetic.st) {
    case WAIT_FALL: /* idle */
        MagProc_Autotune_idle();  // 기존 자동보정 유지

        if (abs_rel >= T_hi) {
            ls.magnetic.st = WAIT_RISE;
            ls.magnetic.debounce_until = t + pdMS_TO_TICKS(DEBOUNCE_MS);
            s_evt_start_tick = t;
            s_evt_area = abs_rel;
            // 래치는 일단 유지(너무 빨리 끄지 말고), 이벤트 종료 시점에 해제
        }
        break;
    ...
    case WAIT_RISE: /* in-event */
        s_evt_area += abs_rel;
        Send_Magnetic_data();
        ls.magnetic.st = WAIT_FALL;

        if (abs_rel < T_lo) {
            ls.magnetic.st = WAIT_FALL;
            ls.magnetic.refractory_until = t + pdMS_TO_TICKS(REFRACT_MS);

            TickType_t dt_ticks = t - s_evt_start_tick;
            uint32_t   dt_ms    = (uint32_t)dt_ticks * portTICK_PERIOD_MS;
            uint8_t ok = (dt_ms >= EVT_MIN_MS) && (s_evt_area >= EVT_MIN_AREA);

            if (ok) {
                ls.magnetic.traffic_count++;
                s_q_true++;
                //Send_Magnetic_data();
                // >>> 속도 계산 복원 (dual-sensor)
                if (peer_tick_or_0 != 0 && Sensor_Distance > 0.0f) {
                    TickType_t dt_peer_ticks = t - peer_tick_or_0;    // wrap-safe
                    float sec = (float)dt_peer_ticks * (portTICK_PERIOD_MS / 1000.0f);
                    if (sec > SPEED_SEC_MIN) {
                        float kmh = (Sensor_Distance / sec) * 3.6f;

                        // 저역통과 필터(선택): α=0.2
                        s_speed_lpf = ( (SPEED_LP_ALPHA_PM * kmh) +
                                        ((1000 - SPEED_LP_ALPHA_PM) * s_speed_lpf) ) / 1000.0f;

                        if (s_speed_lpf < 0.0f)      s_speed_lpf = 0.0f;
                        if (s_speed_lpf > SPEED_KMH_MAX) s_speed_lpf = SPEED_KMH_MAX;

                        ls.magnetic.Speed = (uint8_t)(s_speed_lpf + 0.5f);
                    }
                    // 한 번 쓴 타임스탬프는 소비했다고 표시 (선택)
                    ls.magnetic.Recv_traffic_time = 0;
                }
                // <<< 속도 계산 복원

                // 여기서 UART 직접 송신 대신, “보낼 것” 플래그를 세워 UART 태스크에 위임 권장
                // ls.magnetic.tx_flag_event_done = 1;
            } else {
                s_q_false++;
            }

            s_evt_area = 0;
            s_pre_latch_on = 0; // 이벤트 끝나면 래치 해제
        }
        break;
    }
...
{
	//printf("[logic] logicMagneticTask\r\n");
	const TickType_t tickNow = xTaskGetTickCount();
	if (HAL_GPIO_ReadPin(ADC_TEMP_GPIO_Port, ADC_TEMP_Pin) == GPIO_PIN_SET)
	{	HAL_IWDG_Refresh(&hiwdg);
		//Magnetic_Sensor_Read_stsus_check();
...
void Send_Magnetic_data()
{
    ...
    int16_t rel    = (int16_t)(ls.magnetic.mag_z - (int16_t)ls.magnetic.Z0);
    int16_t absrel = (rel >= 0) ? rel : (int16_t)(-rel);
    int16_t T      = (int16_t)ls.magnetic.T;
    uint8_t state  = (uint8_t)ls.magnetic.st;

	memset((uint8_t*) &send_buf, 0, sizeof(send_buf));

	send_buf[bp++] = 0x7E;								//STX
	//send_buf[bp++] = 0x7E;								//STX
	send_buf[bp++] = 0x14;							//Length~LRC Size
	//uint8_t len_pos = bp++;
	send_buf[bp++] = RF_CMD_SEND;						//OP-Code(Modem Send Opcode)

	send_buf[bp++] = 0x01;								//Master
    ...
    send_buf[bp++] = (rel    >> 8) & 0xFF;
    send_buf[bp++] = (rel)    & 0xFF;
    send_buf[bp++] = (absrel >> 8) & 0xFF;
    send_buf[bp++] = (absrel) & 0xFF;
    send_buf[bp++] = (T    >> 8) & 0xFF;
    send_buf[bp++] = (T)    & 0xFF;
    send_buf[bp++] = state;
...
/*    uint8_t bytes_op_to_now = (uint8_t)(bp - (len_pos + 1));
    uint8_t length_value = bytes_op_to_now + 1;  // + LRC(1)
    send_buf[len_pos] = length_value;*/
