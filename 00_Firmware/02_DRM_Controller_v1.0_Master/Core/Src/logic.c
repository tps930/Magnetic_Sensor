...
void logicIdleTask()
{
	//printf("HAL_IWDG_Refresh Init.....\r\n");
	ls.tick.Tick = xTaskGetTickCount();
...
void logicUartTask()
{
	...
	if(ls.modem.recvFlag == pdTRUE)
	{
		...
		case Time_REQ:
			...
			break;
		case Time_ACk:
			 master_tick = (ls.modem.recvData.data[0] << 24) | (ls.modem.recvData.data[1] << 16) | (ls.modem.recvData.data[2] << 8) | (ls.modem.recvData.data[3]);
		...
		case Magnetic_Time_Send:
			ls.magnetic.Recv_traffic_time = (ls.modem.recvData.data[0] << 24) | (ls.modem.recvData.data[1] << 16) | (ls.modem.recvData.data[2] << 8) | (ls.modem.recvData.data[3]);
...
/*	if (ls.stat.isRegisterCall && ls.ble.isInit == pdTRUE) {
	...
	if (ls.stat.isStatusCall) {
		if (CalcTickDiff(ls.tick.Send, ls.tick.Tick) > 100) {
