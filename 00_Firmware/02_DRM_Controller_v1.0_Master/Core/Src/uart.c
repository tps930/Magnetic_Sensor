...
void USART1_IRQHandler(void)
{
	  HAL_UART_IRQHandler(&huart1);
		...
		{
			s.uartData[COMM_UART1].rxTicks = ls.tick.Tick;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		s.uartData[COMM_UART4].rxTicks = ls.tick.Tick;
	...
	{
		s.uartData[COMM_UART5].rxTicks = ls.tick.Tick;
...
void UART_Baud(UART_HandleTypeDef* huart, uint32_t baud)
{
	...
	if(huart->Instance==USART1)
	{
		HAL_UART_Receive_DMA(huart, &s.uartData[COMM_UART1].rxCh, 1);
	}else if(huart->Instance==USART4)
	{
		HAL_UART_Receive_DMA(huart, &s.uartData[COMM_UART4].rxCh, 1);
	}else if(huart->Instance==USART5)
	{
		HAL_UART_Receive_DMA(huart, &s.uartData[COMM_UART5].rxCh, 1);
	}
...
void BLE_Data_Parse()
{
	...
	pos = strtok_r(recvptr, " :\r\n", &next_pos);

	if (strncmp(pos, "+OK", 3) == 0) {
		ls.ble.isStatus = pdTRUE;
		return;
	}
	...
	if (strncmp(pos, "+READY", 6) == 0) {
		ls.ble.BaudReady = pdTRUE;
		return;
	}

	if(ls.ble.isInfoCall == pdTRUE)
	{
		for (int i = 0; i < 6; i++) {
			ls.ble.mac_address[i] = strtol(pos, NULL, 16);
			pos = strtok_r(NULL, ":,", &next_pos);
		}
		printf("BLE_MacAddress [%02X:%02X:%02X:%02X:%02X:%02X]\r\n",
				ls.ble.mac_address[0], ls.ble.mac_address[1],
				ls.ble.mac_address[2], ls.ble.mac_address[3],
				ls.ble.mac_address[4], ls.ble.mac_address[5]);

		ls.ble.isInfoCall = pdFALSE;
	}
...
void Modem_Data_Parse()
{
    ...
    memset(&uartData->packet, 0x00, RX_BUFLEN);
    memset(&ls.modem.recvData, 0x00, sizeof(COMMAND_T));
    ...
    if(recvLrc != 0xFF){
    	if (recvLrc != calcLrc) {
			...
			ls.modem.recvData.Result = pdFALSE;
			return;
    	}
   	}
/*	packet.opcode = uartData->packet[1];
    ...
    ls.modem.recvData.opcode = uartData->packet[1];
	memcpy(&ls.modem.recvData.data, &uartData->packet[2], 4);
/*    ls.modem.recvData.opcode = uartData->packet[1];

    switch (ls.modem.recvData.opcode)		//Opcode
    {
    	case RF_CMD_BOOT_MSG:		//초기 부팅시 수신 메세지
    		ls.modem.recvData.Result = pdTRUE;
    		memcpy(&ls.modem.recvData.data, &uartData->packet[2], 2);		//Section-ID, Device-ID
    		ls.modem.recvFlag = pdTRUE;
    		break;

    	case RF_CMD_REG_STATUS:		//C-Resigter 상태 정보
    		ls.modem.recvData.Result = pdTRUE;
    		memcpy(&ls.modem.recvData.data, &uartData->packet[2], 11);
    		ls.modem.recvFlag = pdTRUE;
    		break;
    	...
    	case RF_CMD_RECV:			//Recv Data Packet
    		ls.modem.recvData.Result = pdTRUE;
    		ls.modem.recvData.datalen = uartData->packet[1];
    		memcpy(&ls.modem.recvData.data[0], &uartData->packet[2], ls.modem.recvData.datalen - 2);
    		ls.modem.recvFlag = pdTRUE;
    		break;

    	default:
    		ls.modem.recvData.Result = pdFALSE;
    		printf("\r\n[Modem] OP_CODE Error[%2X]\r\n", ls.modem.recvData.opcode);
    		ls.modem.recvFlag = pdTRUE;
    		break;
    }*/
}
...
void Modem_Receive_Check()
{
	...
	if (uartData->rxProcPtr != 0 && CalcTickDiff(uartData->rxTicks, ls.tick.Tick) > 1000) {
		uartData->rxTicks = ls.tick.Tick;
...
void Firmware_Data_Parse()
{
    ...
    memset(&uartData->packet, 0x00, RX_BUFLEN);
    memset(&ls.ble.recvData, 0x00, sizeof(COMMAND_T));

    memcpy(&uartData->packet,  &uartData->rxDataBuff[0], uartData->rxSize);

    printf("[Modem] Ble_Data_Parse\r\n");
	...
	ls.ble.recvData.opcode = uartData->packet[0];

    switch (ls.ble.recvData.opcode)		//Opcode
    {
    	case FIRMWARE_CMD_REG_REQ:		//모뎀 상태 요청
			printf("[Firmware] Register Data Request \r\n");
    		ls.ble.recvData.Result = pdTRUE;
    		ls.ble.recvFlag = pdTRUE;
    		break;

    	case FIRMWARE_CMD_REG_SET:		//C-Resigter 설정
			printf("[Firmware] Register Data Setting \r\n");

    		ls.ble.recvData.Result = pdTRUE;
    		memcpy(&ls.ble.recvData.data, &uartData->packet[5], 8);
    		ls.ble.recvFlag = pdTRUE;
    		break;
    	...
    	default:
    		ls.ble.recvData.Result = pdFALSE;
